...
# on mismatch.
#
# The zip is reproducible on a given build environment (fixed entry
# order, mtimes, permissions, line endings and creator system): rebuilt
# there, its sha1 only changes when the content changes. Builds on a
# different zlib/Python may yield a different (equally valid) sha1 —
