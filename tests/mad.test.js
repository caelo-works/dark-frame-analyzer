// histogramMAD against the exact reference MAD (sorted deviations)
...
function refMAD(values) {
   const med = m.arrayMedian(values);
...
// Gaussian polluted by 2% violent hot pixels — the case that biased avgDev
...
for (const [name, values] of Object.entries(cases)) {
   ...
   const got = m.histogramMAD(toHistogram(values), ref.median);
...
// Empty histogram → 0
assert.strictEqual(m.histogramMAD(new Array(65536).fill(0), 0), 0);

console.log("mad: histogramMAD matches the exact MAD on 4 distributions");
